...
const TopFive = ({ topPost }) => {
  ...
  const nowAt = displayedAt(new window.Date(topPost.createdAt));

  return (
    <TopFiveContainer>
      <TopFiveSection1>
        <NickName>{topPost.author}</NickName>
        <Date>{nowAt}</Date>
      </TopFiveSection1>

      <TopFiveSection2>
        <Job>{topPost.jobGroup}</Job>
        <Title>{topPost.title}</Title>
      </TopFiveSection2>

      <TopFiveSection3>
        <Content>{topPost.content}</Content>
        <Image>
          <img
            src={
              topPost.image !== ''
      ...
      <TopFiveSection4>
        <div className="section4">
          <Heart>
            {topPost.postHeartYn ? <FillLike /> : <Like />}
            <div className="heart">{topPost.postHeartCnt}</div>
          </Heart>
          <Comment>
            <svg
              width="19"
            ...
            <div className="comment">{topPost.commentCnt}</div>
          </Comment>
        </div>
        <Hit>조회수 {topPost.hit}</Hit>
      </TopFiveSection4>
    </TopFiveContainer>
  );
};
